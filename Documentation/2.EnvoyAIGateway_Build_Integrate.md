...
- [Envoy AI Gateway / ExtProc / Controller](#what-is-envoy-ai-gateway--extproc--controller)
...
- It introduces an **External Processor (ext_proc) gRPC server**
...
scripts/envoy.sh start | restart | status

# 3. Status of clusters(openai_cluster, extproc_cluster) from envoy
...
The counter starts at **0** after every Envoy restart and only increments when HTTP traffic passes through the listener on `:8080` (not when you query `/stats` alone).

**Two-step check (before → request → after):**

```bash
# 1. Baseline (0 is normal if no LLM traffic since last Envoy start)
curl -s http://127.0.0.1:9901/stats | grep '^http.llm_ingress.ext_proc.streams_started:'
# http.llm_ingress.ext_proc.streams_started: 0

# 2. Send one request through Envoy (Authorization not required — gateway injects key)
curl -s http://127.0.0.1:8080/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{"model":"gpt-4o-mini","messages":[{"role":"user","content":"hi"}],"max_tokens":1}'

# 3. Counter should increase by 1 (429 from OpenAI still counts — ext_proc ran)
curl -s http://127.0.0.1:9901/stats | grep '^http.llm_ingress.ext_proc.streams_started:'
# http.llm_ingress.ext_proc.streams_started: 1
```

**Also confirms gRPC to AI Gateway:**

```bash
curl -s http://127.0.0.1:9901/clusters | grep 'extproc_cluster::127.0.0.1:1063::rq_total'
# rq_total increases with each LLM request (typically +2 per chat: listener + upstream ext_proc)
```
